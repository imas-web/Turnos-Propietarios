...
router.get(
  ...
  ah(async (req, res) => {
    const { rows } = await pool.query(
      'SELECT * FROM propietarios WHERE creado_por = $1 ORDER BY nombre ASC',
      [req.usuario.sub]
    );
    res.json(rows);
  })
...
router.post(
  ...
  ah(async (req, res) => {
    ...
    const { rows } = await pool.query(
      `INSERT INTO propietarios (nombre, email, telefono, unidad, creado_por)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [nombre, email, telefono || null, unidad, req.usuario.sub]
    );
...
router.delete(
  ...
  ah(async (req, res) => {
    const { rows } = await pool.query(
      'DELETE FROM propietarios WHERE id = $1 AND creado_por = $2 RETURNING id',
      [req.params.id, req.usuario.sub]
    );
    if (!rows[0]) return res.status(404).json({ error: 'Propietario no encontrado' });
    res.status(204).send();
